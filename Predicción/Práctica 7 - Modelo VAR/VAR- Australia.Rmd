...
#Objetivo
1- Elegir 2 BBDD de un país y realizar un modelo VAR 
2- GPD Y CPI de Australia (ambas están en quartiles y previamente ajustadas)
...
```{r}
library(tidyverse)
library(quantmod) #Acceder a los datos
library(forecast)
```

#Series 
```{r}
series = c('NAEXKP01AUQ657S', 'AUSCPALTT01IXOBSAQ') # FRED codes for US GDP growth and CPI
...
Calculamos tanto el PIB como el ratio de inflacción anual, guardándolo en Data para poder trabajar con ella. 
```{r}
