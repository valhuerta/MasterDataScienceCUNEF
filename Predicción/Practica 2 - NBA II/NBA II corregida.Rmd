...
#Con "HERE" no necesito poner toda la direccion donde este. Solo con lo de arriba vale
#Es la raiz y luego el nombre del archivo 
```
...
skim(raw_data)
#Con esta funcion te muestra una vision detallada de los datos.
```
...
#%<>% lo que quire decir es que lo que tienes en la izq de eso entra dentro de la funcion, como pone <> entre medias significa igualdad que es que me lo meta en esa funcion.  
...
factores <- c("Player","NBA_Country","Tm")
...
# Other Correlations
#Las estrellas en rojo llo que indica es que si es distinta de 0, es decir cuales estan correlacionadas. Las lineas rectas determinan que no hay relacion entre ellas 
...
```{r Log salary}
#Sabiendo el logaritmo de Salary conocemos lo que cambia 
...
```{r Regsubsets}
#Quita la tres varibles factor (equipo, pais, y la otra que habia)

nba <- raw_data %>% select_at(vars(-vars))


#ha cogido el modelo secuencial 

model_select <- regsubsets(Salary~. , data =nba, method = "seqrep",nvmax=24)

model_select_summary <- summary(model_select)

# hace el dta frame para que me enseñe todos los resultados del modelo 
data.frame(
...
#ha elegido el R^2 ajustado pero podria haber cogido cualquier otro 

plot(model_select, scale = "adjr2", main = "Adjusted R^2")
#Con este grafico lo que ve es que el modelo que mejor ajusta es el que esta arriba del todo que es el que tiene el r^2 mejor. 
...
#Ahora hace el Cross Validation, lo hace mediante el enlace que hay arriba. 
#
...
cv.errors <-  map(model.ids, get_model_formula,model_select , "Salary") %>%
  ...
  unlist()
cv.errors #Es el mean square error. Son muy altos porque es Salario
...
# all sample
  #Ahora hago lm con toda la muestra. 
...
#hace todo lo de antes que he hecho a mano, pero de una vez. 
#HTF es el metodo, lo pone en la libreria. 
...
model_bestglm2 <-
    bestglm(Xy = nba_bestglm,
            family = gaussian,
            IC="LOOCV", 
            t=10, #para determinar el tamaño, no afecta que lo cambie 
             method = "exhaustive")
...
#Usa directamente elastic net, que debemos saber lo que es 
...
#coge la muestra, la divide en train y test
set.seed(6789)
